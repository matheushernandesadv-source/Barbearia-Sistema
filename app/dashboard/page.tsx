...
export default function OwnerDashboard() {
  return (
    <>
      <Topbar title="Painel do Proprietário" subtitle="Visão geral de hoje, 13 de julho" />

      <div className="p-5 lg:p-8 space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Faturamento hoje" value={formatBRL(ownerSummary.revenueToday)} icon={DollarSign} trend="+12% vs ontem" />
          <StatCard label="Clientes atendidos" value={String(ownerSummary.clientsToday)} icon={Users} trend="+4 vs ontem" />
          <StatCard label="Serviços vendidos" value={String(ownerSummary.servicesSoldToday)} icon={Scissors} trend="+6%" />
          <StatCard label="Produtos vendidos" value={String(ownerSummary.productsSoldToday)} icon={ShoppingBag} trend="-2%" trendTone="red" />
          <StatCard label="Lucro estimado" value={formatBRL(ownerSummary.estimatedProfit)} icon={TrendingUp} trend="+8%" />
          <StatCard label="Caixa disponível" value={formatBRL(ownerSummary.cashAvailable)} icon={Wallet} hint="Atualizado agora" />
          <StatCard label="Barbeiro destaque" value={ownerSummary.topBarberToday} icon={Trophy} trendTone="neutral" hint={formatBRL(3990)} />
          <StatCard label="Faltas hoje" value={String(ownerSummary.noShowsToday)} icon={UserX} trendTone="red" hint="2 clientes" />
        </div>
